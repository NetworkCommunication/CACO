...
import pandas as pd
from t_alltime_env_ddpg import ENV_DDPG
from t_alltime_env_ddpg_local import ENV_DDPG_LOCAL
from t_alltime_env_ddpg_nogan import ENV_DDPG_NOGAN
from t_alltime_env_ddpg_nocache import ENV_DDPG_NOCACHE
from network import Agent
import numpy as np

def ddpg(episode, step, num, T):
    ...
    df.to_excel("T_alltime/T_alltime_ddpg/T_alltime_ddpg_{}_{}.xlsx".format(T, num))
...
def ddpg_local(episode, step, num, T):
    # tcar为需卸载车辆， scar为服务车辆
    num_car = 21
    num_scar = int(num_car * 1 / 3)
    num_tcar = num_car - num_scar
...
def ddpg_nogan(episode, step, num, T):
    # tcar为需卸载车辆， scar为服务车辆
    num_car = 21
    num_scar = int(num_car * 1 / 3)
    num_tcar = num_car - num_scar
...
def ddpg_nocache(episode, step, num, T):
    # tcar为需卸载车辆， scar为服务车辆
    num_car = 21
    num_scar = int(num_car * 1 / 3)
    num_tcar = num_car - num_scar
...
if __name__ == '__main__':
    ...
    num = 1
    T = 2
    ddpg(episode, step, num, T)
    ddpg_local(episode, step, num, T)
    ddpg_nogan(episode, step, num, T)
    ddpg_nocache(episode, step, num, T)