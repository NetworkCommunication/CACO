...
def ddpg(episode, step, num):
    ...
    for i in range(episode):
        ...
        while not done:
            ...
            score += reward
            obs = new_state
            num_unload += env.num_unload
            num_unload_success += env.num_unload_success
        episode_record.append(i)

        if num_unload == 0:
            rate_record.append(0)
        else:
            rate_record.append(num_unload_success / num_unload)
    ...
    df = pd.DataFrame({"Episode": episode_record_step, "Rate": rate_record_step}).set_index('Episode')
    df.to_excel("episode_rate/episode_rate_ddpg/episode_rate_ddpg_{}.xlsx".format(num))
...
def ddpg_nocache(episode, step, num):
    ...
    df = pd.DataFrame({"Episode": episode_record_step, "Rate": rate_record_step}).set_index('Episode')
    df.to_excel("episode_rate/episode_rate_ddpg_nocache/episode_rate_ddpg_nocache_{}.xlsx".format(num))
...
def ddpg_nogan(episode, step, num):
    ...
    df = pd.DataFrame({"Episode": episode_record_step, "Rate": rate_record_step}).set_index('Episode')
    df.to_excel("episode_rate/episode_rate_ddpg_nogan/episode_rate_ddpg_nogan_{}.xlsx".format(num))
